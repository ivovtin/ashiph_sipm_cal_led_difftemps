...
const float hmint = -12E-9, hmaxt = 80E-9, hshift = -100E-9; //область интегрирования , hshift сдвижка для шумов, секунды
...
Double_t binPed1 = 450.;
Double_t binPed2 = 550.;

Double_t binSig1 = 560.;
Double_t binSig2 = 660.;
...
void histo_new(const char* dirname,  int channel = 1, TString outfile = "histogram.root" ) //
{
   ...
   //нарисуем несколько гистограмм с осциллограммами для примера.
    ...
    Double_t bHistInt1 = 80000;
    Double_t bHistInt2 = 90000.;
    ...
    for (long unsigned int i = 0; i < maxoscnum; i++) { 
		...
		h->SetBinContent(j,voscWave[i][j]) ;         
            ...
            h->Fit("fitl","LQ","",binPed1, binPed2);
            Double_t p0 = fitl->GetParameter(0);
   	    
	    //ped area
            h->GetXaxis()->SetRangeUser(binPed1, binPed2);
	    ...
	    h->GetXaxis()->SetRangeUser(binSig1, binSig2);
	...
	c->cd(4);	
		...
		TF1* fitlCor = new TF1("fitlCor","pol0",binPed1, binPed2);
                h->Fit("fitlCor","LQ","",binPed1, binPed2);
	...
	int firstbinPed = hAmpPedCor->GetXaxis()->FindBin(bHist1);
        ...
        float integral_afterPed = hAmpPedCor->Integral(binBorder1,lastbinPed);
        float integral_allPed = hAmpPedCor->Integral(firstbinPed,lastbinPed);
	...
	cout<<"Amp 1ph.e. = " << (hAmpSigCor->GetMean() - g1)/Npe <<" +/- "<<NpeErr<<endl;
        ...
        err_Freq = sqrt(integral_afterPed*(integral_allPed-integral_afterPed)/pow(integral_afterPed,3))/(binPed2-binPed1)*pow(10,9);
    ...
    */
    ...
    c->SaveAs(TString(dirname) + "/" + "res_canvas.root" );     
    ...
    SfileOut << Npe << "\t" << NpeErr << "\t" << Freq << "\t "<< err_Freq << "\n";
    SfileOut.close();
